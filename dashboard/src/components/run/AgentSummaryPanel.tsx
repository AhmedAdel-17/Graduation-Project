/**
 * Inline left-side summary panel — replaces the slide-out Sheet drawer
 * when an agent card is clicked in the timeline.
 *
 * Re-uses the same agent-specific renderers from AgentDetailDrawer so the
...
// Re-export the renderer logic — we can't import private functions from the
// drawer, so this is a self-contained copy. If/when both files need updates,
// keep them in sync.
        ...
        <CardContent>
          <div className="flex h-40 items-center justify-center text-center">
            <p className="text-sm text-muted-foreground">
              Click any agent card in the pipeline
              <br />
              to see its analysis here.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="h-full overflow-y-auto">
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
        <div>
          ...
          <Badge variant="outline" className={`mt-1 text-[10px] ${statusColor}`}>
            {agent.status}
          </Badge>
        </div>
...
/** Pick the right human-readable layout per agent type (mirrors AgentDetailDrawer). */
...
// ───────────── Renderers (duplicated from AgentDetailDrawer for now) ──────
...
function ResearchManagerView({ d }: { d: any }) {
  ...
  if (!text) {
    return <p className="text-sm italic text-muted-foreground">No reasoning recorded.</p>;
  }
  return (
...
function RiskDebatorsView({ d }: { d: any }) {
  ...
  if (!history) {
    return <p className="text-sm italic text-muted-foreground">No debate output recorded.</p>;
  }
  return (
    <p className="whitespace-pre-line text-sm leading-relaxed">{history}</p>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      {clauses.length > 0 && (
        <>
          <Divider />
          <SectionTitle>Constitution clauses</SectionTitle>
          <div className="flex flex-wrap gap-1">
