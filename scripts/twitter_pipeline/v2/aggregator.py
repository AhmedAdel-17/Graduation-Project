...
Each post contributes based on its model sentiment score and a production
weight stack:

    weighted_score = sentiment_score
                     * (entity_confidence * source_weight * intent_weight * content_weight)
                     * log(1 + engagement)

The split step always returns a usable market signal. Small samples reduce the
final confidence instead of producing a hard NO_SIGNAL failure.
"""
...
class ScoredPost:
    ...
    intent: dict     # IntentResult.to_dict()
    content: dict    # ContentType.to_dict()
    sentiment: dict  # {label, score, confidence, model_used}
...
def split_outputs(
    ...
    """
    Returns the legacy payload plus production-friendly stock arrays.
    """
    ...
    ranked_signals = sorted(per_stock.values(), key=_sort_key, reverse=True)
    all_stocks = [_to_stock_card(signal) for signal in ranked_signals]
...
def render_summary(per_symbol: Dict[str, dict]) -> List[str]:
    """Return a list of pretty lines for logging."""
    lines = []
    if not per_symbol:
        return ["(no signals)"]

    market = per_symbol.get("EGX_MARKET")
    rest = [value for key, value in per_symbol.items() if key != "EGX_MARKET"]
    rest.sort(key=lambda value: -value["n"])

    if market:
        lines.append(
            f"  [MARKET]   EGX  n={market['n']:3d}  "
            f"sent={market['weighted_sentiment']:+.2f}  "
            f"conf={market['confidence']:.2f}  -> {market['label'].upper()}"
        )

    for value in rest:
        intents = ",".join(
            f"{label}:{count}"
            for label, count in value["intent_breakdown"].items()
            if label != "NONE"
        )
        lines.append(
            f"  [{value['symbol']:<6}]   n={value['n']:3d}  "
            f"sent={value['weighted_sentiment']:+.2f}  "
            f"conf={value['confidence']:.2f}  -> {value['label'].upper():<7}  "
            f"({intents or 'no-intent'})"
        )
    return lines
