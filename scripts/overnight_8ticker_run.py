#!/usr/bin/env python3
"""Overnight 8-ticker validation runner with per-ticker timeout and retry.

Usage:
    python3 scripts/overnight_8ticker_run.py

Runs each ticker sequentially with a 45-minute timeout per ticker.
On failure/timeout, retries once. Logs to backtest_results/overnight_run.log.
"""
...
TICKERS = [
    "ETEL.CA", "TMGH.CA", "EAST.CA", "FWRY.CA",
...
TIMEOUT_SECONDS = 45 * 60  # 45 minutes per ticker
MAX_RETRIES = 1
...
def find_report(ticker: str, after_ts: float) -> str | None:
    """Find the most recent report for ticker created after after_ts."""
...
def run_ticker(ticker: str) -> dict:
    """Run a single ticker backtest. Returns result dict."""
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                if report:
                    ...
                    result["error"] = "Process succeeded but no report file found"
                    log(f"  {ticker} process OK but no report file found")
            else:
...
def main():
    log("=" * 60)
    log("Overnight 8-ticker validation run")
    log(f"Tickers: {', '.join(TICKERS)}")
    log(f"Config: interval=14, benchmark=EGX30, start=2024-01-01, end=2024-06-30")
    ...
    manifest = {
        ...
        "comi_r4_report": "backtest_results/report_COMI.CA_20260625_043132.json",
        "results": results,
    }
    manifest_path = RESULTS_DIR / "overnight_manifest.json"
