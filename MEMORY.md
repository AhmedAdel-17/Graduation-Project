...
### 0. Quick prediction / dashboard 401 from DeepSeek — **CRIT** — RESOLVED (2026-05-19)
- **Symptom:** Dashboard "Quick analysis" and the first stage of "Run full pipeline" crashed with `openai.AuthenticationError 401 — invalid api key ****0qfP` against `https://api.deepseek.com`. Backtests launched from `/backtests/run` were unaffected because `TradingAgentsGraph.__init__` already resolves the right key per-backend.
- **Root cause:** `run_egx_prediction.py` (`analyze_ticker_for_api` and `run_prediction`) and `tradingagents/agents/profiling/investor_profiling_agent.py:_build_llm` constructed `ChatOpenAI(base_url="https://api.deepseek.com", ...)` without passing `api_key=`. langchain_openai then defaulted to `OPENAI_API_KEY`, which `.env` had set to the Groq key (`gsk_...`). Groq key against DeepSeek endpoint → 401.
- **Fix:** Added a `_resolve_llm_api_key(backend_url)` helper that selects `DEEPSEEK_API_KEY` / `OPENROUTER_API_KEY` / `GROQ_API_KEY` / `OPENAI_API_KEY` based on the backend URL substring (same pattern already used in `trading_graph.py:114`). Both `ChatOpenAI(...)` constructions in `run_egx_prediction.py` and the one in `investor_profiling_agent.py` now pass that key explicitly.
- **Prevention:** any new `ChatOpenAI(base_url=...)` site must pass an explicit `api_key=` resolved from the backend, never rely on `OPENAI_API_KEY`. Grep `ChatOpenAI(` periodically — there are currently three resolution sites; keep them consistent.
...
### C. Look-ahead bias in `scripts/backtester.py` — **RESOLVED (2026-05-20)**
- **C1.** ~~`_evaluate_trade_outcomes()` labels trades WIN/LOSS using forward prices fetched after the backtest, then surfaces this as "Hit Rate (fwd)".~~ **RESOLVED.** Function deleted along with its caller and the `outcome_summary` block in `save_results`. Win rate is now computed by `tradingagents.rl.walkforward._closed_trade_winrate` from realized PnL on SELL trades only; `_wilson_ci` provides a 95% confidence band so the number is honest on small samples. The metrics dict exposes `Win Rate CI Lo`, `Win Rate CI Hi`, `Closed Trades`. Regression gate `tests/test_backtester_robustness.py::test_no_lookahead_functions`.
- **C2.** ~~Reflection inside the loop — `scripts/backtester.py:855-881` calls `graph.reflect_and_remember(returns_losses)` per date with realized PnL.~~ **RESOLVED (DB-infra PR 7).** Note: the follow-up `_flush_reflection_with_forward_returns` helper added in PR 7 was also deleted in this PR because it consumed the look-ahead forward returns produced by `_evaluate_trade_outcomes`. Backtests no longer write reflection memory; production realized live trades are the only legitimate training signal. PR-7's structural regression gate (`graph.reflect_and_remember(` absent from `scripts/backtester.py`) is still in force at `tests/test_reflection_batch.py`.
- **C3.** ~~Risk-free rate hardcoded to 0.05.~~ **RESOLVED.** `BacktestingEngine.__init__` reads `config["egx_risk_free_rate"]` (default 0.275 — CBE policy proxy in `default_config.py:67`) and falls back to `walkforward.default_risk_free_rate()` (0.24) when the key is absent. The metrics dict and the saved JSON report both carry `risk_free_rate_used` so the number is auditable. Regression gates: `tests/test_backtester_robustness.py::test_risk_free_rate_default_is_not_005` and `::test_risk_free_rate_used_in_sharpe`.
- **C4.** ~~Benchmark window misalignment — `nearest-earlier-date` fuzzy match.~~ **RESOLVED.** Replaced with strict date intersection between `daily_history` and `_bm_data_map`. New `_align_benchmark_to_strategy()` emits a structured `benchmark` block: `name`, `first_aligned_date`, `last_aligned_date`, `n_aligned_days`, `coverage_pct`, `total_return_pct`, `annualized_return_pct`, `alpha_pct`, `tracking_error_pct`. Coverage below 80% logs a warning but still reports the alpha (with the coverage tag attached). Regression gate `tests/test_backtester_robustness.py::test_align_benchmark_intersects_dates_only`.
...
### R. v2 entity registry too narrow — **MED** — PARTIALLY RESOLVED (2026-05-18)
- **Original:** `scripts/social_pipeline/v2/entities.py::SYMBOL_REGISTRY` had thin coverage; live FB groups mention small-caps not in the registry → `symbols_covered = 0`.
- **Resolved 2026-05-18 (social_v2 integration):** Registry rebuilt in `tradingagents/dataflows/social_v2/entities.py` with 84 issuers (EGX-30 + most of EGX-70 + select small-caps), full Arabic + English alias coverage, and added `QNBA` + `ESRS` (were missing). The scripts/social_pipeline/v2 directory is now the LEGACY copy; the agent path only consumes the new package.
- **Remaining:** Expand to the full EGX main-market top-200 (~100 more issuers) for coverage parity with the regulator universe. Iterate the AR alias lists based on first-month archive contents.
...
### AA. Bear researcher produces empty output — **RESOLVED 2026-05-20**
- **Where:** the graph wiring, not the bear node. Both `graph/setup.py` and `ablation/runner._rebuild_graph` (the builder the *backtester* actually runs) wired the debate as a Bull↔Bear **cycle** via `should_continue_debate` conditional edges, with a non-deferred "Analysts Sync" fan-in barrier.
- **Root cause (confirmed via per-node tracing 2026-05-20):** "Analysts Sync" had 4 incoming edges and fired once per super-step in which any analyst's "Msg Clear" edge delivered a trigger. Analysts finish in different super-steps (variable tool-call rounds), so the barrier fired multiple times and re-triggered "Bull Researcher". A second Bull task then ran **concurrently** with the Bear task; both write `investment_debate_state`, and the `_keep_last` reducer kept Bull's write (which passes through `bear_history=""`), discarding Bear's real 15–20 KB thesis. Result: `bear_history` 0 chars, `bear_thesis` null on every run.
- **Fix (commit pending):** debate rewired as a strict linear chain `Bull → Bear → Research Manager` in both `graph/setup.py` and `ablation/runner.py` (no cycle = no concurrent re-trigger); "Analysts Sync" given `defer=True` so it is a true barrier. Functionally equivalent to prior intent — `ConditionalLogic()` is always built with `max_debate_rounds=1`, so the debate was always a single round. Also hardened `memory.get_memories()` to fall back to BM25 instead of crashing the agent node on a ChromaDB `InternalError` (empty/corrupt HNSW segment), which was separately crashing the Bull node.
- **Verified:** backtest of COMI.CA 2024-01-02 now produces `bear_history` ≈ 17 KB and a fully populated `bear_thesis` (recommendation `reduce`, conviction `high`, COMI-specific `key_risks`). The Research Manager now judges a real two-sided debate instead of inventing a generic bear case.
- **Follow-up:** the dashboard Bear card's amber "no structured thesis" warning can now be removed; multi-round debate (`max_debate_rounds>1`) would need a real round-counter node, not a self-cycle.
...
- **Social sentiment v2 promoted to agent path + backtest honesty + post archive (2026-05-18)** — Replaced the dead `scripts/social_pipeline/v2/` standalone (broken imports — `models.py`, `relevance.py`, `sentiment.py` deleted in §250 cleanup but never replaced) with a self-contained production package at `tradingagents/dataflows/social_v2/`. **Module map:** `models.py` (Post dataclass), `relevance.py` (Layer-0 EGX gate), `entities.py` (84-ticker SYMBOL_REGISTRY w/ EN+AR aliases incl. new `QNBA`/`ESRS`), `intent.py`, `content_type.py`, `quality_gate.py`, `aggregator.py` (now also tracks `n_distinct_sources` + `n_distinct_days` per symbol), `sentiment_runner.py` (wraps `tradingagents.utils.sentiment_engine` for FinBERT/CAMeLBERT/XLM-R + VADER baseline), `cache.py` (diskcache, 1 h Apify TTL + 30 min signal TTL), `post_store.py` (best-effort Postgres archive), `pipeline.py` (7-stage orchestrator: SCRAPE→RELEVANCE→ENRICH→GATE→SENTIMENT→AGGREGATE→ARCHIVE), `signal_adapter.py` (the only agent-facing surface), `sources/facebook_apify.py` (Apify actor `2chN8UQcH1CfxLRNE`, 5 user-confirmed AR Facebook groups baked in as `DEFAULT_GROUP_URLS`, stale-cache fallback on HTTP 402/403/429/5xx), `sources/reddit_targeted.py`. **Wiring:** `graph/prefetch.py::_fetch_social_sentiment` and `agents/utils/social_media_tools.py::get_social_sentiment` both call `fetch_v2_signal_json(ticker, curr_date)` first; v1 path retained only as exception fallback. **Backtest honesty:** `signal_adapter._is_backtest()` flags any `curr_date` >1 day in the past; it first attempts archive replay (querying `social_v2_posts`); if <20 historical rows it returns NO_SIGNAL with `reason="historical_proxy_only"` and `gate="backtest.no_archive"`, which `social_media_analyst._try_build_*_sentiment` reconstructs into NO_SIGNAL layer objects so `blend_sentiment` correctly forces pass-through (1.0 × 1.0). **V1 aggregator hardening:** `dataflows/social_media_sources/aggregator.py::PLATFORM_SOURCES` now only contains `{"reddit"}` — `telegram` (public previews ≈ 0 EGX coverage) and `stocktwits` (no EGX symbols) unwired. **DB schema:** appended `social_v2_posts` table to `db_schema.sql` (UNIQUE post_hash, GIN index on `symbols TEXT[]`, post_timestamp btree). Schema is also auto-created by `post_store._connect()` for convenience. **Secrets:** `APIFY_API_TOKEN` and a commented `EGX_FB_GROUP_URLS` template added to `.env`; `tradingagents/dataflows/social_v2/_cache/` added to `.gitignore`. **Smoke tests pass:** package imports clean, `fetch_v2_signal('COMI.CA', '2025-01-01')` round-trips through `_try_build_market_sentiment` / `_try_build_sector_sentiment` / `_try_build_macro_sentiment` to typed `NO_SIGNAL` objects, `blend_sentiment` returns (1.0, 1.0). **ROTATE THE PASTED APIFY KEY** (`apify_api_cTUGlIu...`) — it was disclosed in chat. **Remaining caveats:** real backtests need ≥3 months of `social_v2_posts` rows accumulating before historical replay is meaningful; entity registry still <100% of EGX main market (see §R); the legacy `scripts/social_pipeline/v2/` tree is now dead code and can be deleted in a follow-up.
...
- **Backtester reliability + EGX30 evaluation harness (2026-05-20)** — Closes the §C cluster (C1/C3/C4) and the original Workstream-A crash classes flagged in the chat audit. **Look-ahead removal (§C1):** `_evaluate_trade_outcomes()` deleted; `_flush_reflection_with_forward_returns()` deleted (depended on §C1's forward returns); `outcome_summary` block removed from `save_results`; `Hit Rate (fwd)`, `Outcome WIN/LOSS/NEUTRAL/PENDING`, `trade_result`, and `forward_return_*` no longer appear anywhere in the saved JSON or split-metrics. Honest replacement: `tradingagents.rl.walkforward._closed_trade_winrate` + `_wilson_ci` (already-existing helpers) — `_calculate_metrics()` now exposes `Win Rate`, `Win Rate CI Lo`, `Win Rate CI Hi`, `Closed Trades`. **Risk-free rate (§C3):** `BacktestingEngine.__init__` reads `config["egx_risk_free_rate"]` (default 0.275 from `default_config.py:67`); falls back to `walkforward.default_risk_free_rate()` (0.24). `risk_free_rate_used` is persisted into both `metrics` and the top-level report. **Benchmark alignment (§C4):** new `_align_benchmark_to_strategy()` does a strict date-intersection between `daily_history` and `_bm_data_map`, drops the nearest-earlier fuzzy match. New `benchmark` block in the report carries `name`, `first/last_aligned_date`, `n_aligned_days`, `coverage_pct`, `total_return_pct`, `annualized_return_pct`, `alpha_pct`, `tracking_error_pct`. Coverage <80% logs a warning, doesn't abort. **Crash hardening:** the bare `except` swallowing `graph.propagate()` failures is replaced with a typed handler that re-raises `KeyboardInterrupt`/`SystemExit` and records `decision_status="agent_error"` (+ `error_class`, `error_message`) in `audit_log` — phantom HOLDs are no longer invisible. Empty `stock_data["data"]` and `current_price ≤ 0` cases now log a structured skip and continue. `_fetch_stock_data_with_retry` wraps the gateway call with 3-attempt exponential backoff (1s → 8s). **Resumability:** new `_maybe_write_partial(ticker)` writes an atomic per-ticker JSON checkpoint after every loop iteration (success, agent_error, or circuit_halted); new `_load_partial_if_resume(ticker, resume=True)` restores `cash`, `positions`, `pending_cash_settlements`, `prev_prices`, all four history lists, and returns the set of completed dates so the main loop skips them. CLI gains `--resume`. The partial is removed once `save_results` completes. **Multi-ticker eval harness:** new `scripts/evaluate_egx_backtests.py` runs `BacktestingEngine` sequentially across `--tickers ETEL.CA,TMGH.CA` (defaults), then aggregates the JSON reports via `walkforward.compute_arm_metrics`. Emits `eval_results/multi_ticker_summary_<ts>.csv` (per-ticker rows) + `.md` (per-ticker table + pooled summary: mean return, mean Sharpe, mean alpha vs EGX30, pooled Wilson-CI win rate, tickers beating EGX30, tickers with positive Sharpe). Resumable (skips tickers with a matching report already on disk). **Tests:** `tests/test_backtester_robustness.py` (9 tests, all green) covers the §C1 deletion gates, §C3 sharpe response to risk-free rate, §C4 date-intersection alignment, retry success/give-up paths, and partial-checkpoint roundtrip. `tests/test_reflection_batch.py` slimmed to its still-valid grep-gate (`graph.reflect_and_remember(` absent) plus a new gate that asserts `_evaluate_trade_outcomes` and `_flush_reflection_with_forward_returns` remain deleted. Regression: 96/96 green across the §C-adjacent suites (reflection batch, backtester robustness, RL dataset integrity, RL feature extractor, backtest persistence). **Files:** `scripts/backtester.py` (delta ~+220/-260), `scripts/evaluate_egx_backtests.py` (new, ~360 lines), `tests/test_backtester_robustness.py` (new, 9 tests), `tests/test_reflection_batch.py` (rewritten — 2 grep gates), `MEMORY.md` (this entry + §C cluster moved to resolved). **Reused unchanged:** `tradingagents/rl/walkforward.py` helpers (`_closed_trade_winrate`, `_wilson_ci`, `_max_drawdown`, `_daily_returns`, `compute_arm_metrics`, `default_risk_free_rate`), `tradingagents/dataflows/retry_engine.py` pattern (inlined an equivalent loop because the wrapper is a decorator, not a function call). **Out of scope:** §D universe size (only 2 tickers wired by user choice), walk-forward windows (deferred), broker baseline (chosen single-baseline = EGX30).

- **Decision-priority fix + dashboard insight rework (2026-05-20)** — Follow-up to the entry above, after a dashboard run on `ADIB.CA`/`ETEL.CA` came back all-zeros / one-sided. Three threads:

  **(1) Decision-priority bug.** Backtester runs were silently downgrading every trader BUY to HOLD. Root cause: the old parser's Priority-4 fallback only fired when `risk_assessment.get("approved") == True`, but `risk_assessment` ships as `{}` (the LLM Risk Judge writes its verdict as free text into `risk_debate_state.judge_decision`, not the structured dict), so `approved` was never `True`. The bare-`"HOLD"` string from the judge won Priority 2 and the trader's BUY was discarded. **Fix:** extracted `BacktestingEngine._resolve_decision(final_state, execution_plan)` (static, unit-testable) with an explicit priority order — (1) deterministic veto: `risk_action=="VETO"` OR `risk_assessment.approved is False` OR `critical_violations>0` → HOLD; (2) LLM judge bare action; (3) JSON `{"action":...}` / free-text scan; (4) **trust-the-trader fallback** — when the judge says HOLD, there's no deterministic veto, and the trader's `execution_plan.decision` is BUY/SELL, take the trader's plan. The deterministic safety floor still wins. Every audit row now carries `decision_path` ∈ {`deterministic_veto`, `judge_bare`, `judge_json`, `judge_freetext`, `trader_fallback`, `default_hold`} and `risk_action`. 6 regression tests in `tests/test_backtester_robustness.py` (15 total there now).

  **(2) Dashboard insight rework.** The summary screen showed mechanically-zero metrics (Max DD 0%, Win Rate 0%, Net PnL 0, Veto 0%) whenever positions stayed open, and templated boilerplate instead of real agent text. Changes: `_calculate_metrics()` now *omits* `Benchmark Return`/`Alpha`/`Buy&Hold Return` keys entirely when the underlying data is missing (was writing a misleading `0.00%`). `server/api_server.py:_normalize_llm_report` + `_parse_percent` extended — passes `closed_trades`, `benchmark_return_pct`, `buyhold_return_pct`, `alpha_pct`, the structured `benchmark` block, `buyhold_history`, and `audit_log` through to the dashboard; `_parse_percent` tolerates `"60.00% [95% CI ...]"`. `get_backtest_detail` endpoint also passes `buyhold_history`/`benchmark`/`audit_log`. Dashboard (`dashboard/src/features/backtest/BacktestScreen.tsx`, `services/api/types.ts`): when `closed_trades==0` the Max-DD / Win-Rate tiles are swapped for Open-Positions / Unrealised-P&L; the Judge/Risk agent-card chips drop the confusing `Net PnL`/`Veto rate`/`Max DD`; new **Scenario Comparison** section (Section 03) ranks Strategy vs same-ticker buy-and-hold vs EGX30 buy-and-hold with a "best outcome" badge and a one-line defense verdict; trade-log `PnL` column renamed `Realised P&L`, BUY rows show `Open`.

  **(3) Real agent text surfaced.** The Risk Judge analysis and trader exit plan were always produced but never displayed. Backtester now writes per-date `risk_judge_text` (≤3000 chars from `risk_debate_state.judge_decision`), `bull_thesis_summary` / `bear_thesis_summary` (compacted via new module helper `_summarize_thesis()`), and `bear_thesis_present` into each `audit_log` entry; each BUY trade record gets an `exit_plan` (take-profit ladder, stop-loss, time-stop, invalidation triggers) copied from `execution_plan.exit_logic`. The trade `reasoning` field now prefers real entry-logic timing over the `"Standard execution"` placeholder. Dashboard renders these: Bull/Bear cards show conviction/catalysts/invalidation/upside via new `ThesisDetail`; Risk card shows the Judge's clause-by-clause review via `RiskJudgeText` (expandable); BUY rows in the trade log are click-to-expand revealing `ExitPlanDetail` (TP ladder with gain %, stop-loss with computed max-loss EGP, time-stop, invalidation triggers).

  **EGX30 CSV:** `_load_egx30_csv` candidate list extended with `EGX30ETF ETF Stock Price History.csv` + `EGX30 ETF Stock Price History.csv` (Investing.com ETF-export filenames) alongside the original `EGX 30 Historical Data.csv` / `egx30.csv`.

  **New open issue logged:** §AA — the bear researcher emits empty output (`bear_history` 0 chars, `bear_thesis` null) on every date; debates are one-sided. Interim: dashboard Bear card shows an honest warning. Bug itself unfixed — needs per-node tracing.

  **Verification:** 275/275 Python tests pass (`pytest -m "not integration"`); dashboard `tsc` + Vite build clean; no new lint errors. **Files:** `scripts/backtester.py`, `server/api_server.py`, `dashboard/src/features/backtest/BacktestScreen.tsx`, `dashboard/src/services/api/types.ts`, `tests/test_backtester_robustness.py` (15 tests), `MEMORY.md`, `CLAUDE.md`.
