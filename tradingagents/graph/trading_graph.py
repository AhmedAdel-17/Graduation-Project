...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        # Initialize LLMs
        # seed=42  → deterministic outputs for reproducibility (CLAUDE.md §5.8)
        # max_retries=3 → automatic exponential-backoff on 429/503 errors
        _seed = int(self.config.get("llm_seed", 42))
        _retries = int(self.config.get("llm_max_retries", 3))
        provider = self.config["llm_provider"].lower()

        # Resolve API key from env (DeepSeek is the project default — see
        # CLAUDE.md §4 — but fall back to OPENAI_API_KEY for compatibility
        # with the upstream Tauric framework).
        _api_key = (
        ...
        if provider in ("openai", "ollama", "openrouter"):
            ...
            # ChatAnthropic does not expose a seed parameter yet; temperature=0
            # is the reproducibility lever for Claude models.
            ...
            self.deep_thinking_llm = ChatGoogleGenerativeAI(
                model=self.config["deep_think_llm"],
                temperature=0,
                # Google Gemini supports seed in newer versions via extra kwargs
            )
        ...
        # Optional: wrap in ReliableChatModel for multi-provider failover.
        # Enable with config["enable_llm_failover"] = True (default: off).
        # Requires at least one backup API key (OPENROUTER_API_KEY or GROQ_API_KEY).
        if self.config.get("enable_llm_failover", False):
            try:
                from tradingagents.agents.utils.llm_failover import build_resilient_llm
                import logging as _log
                _log.getLogger("tradingagents").info(
                    "LLM failover enabled — wrapping primary LLMs in ReliableChatModel"
                )
                self.deep_thinking_llm = build_resilient_llm(
                    self.config, role="deep", seed=_seed
                )
                self.quick_thinking_llm = build_resilient_llm(
                    self.config, role="quick", seed=_seed
                )
            except Exception as _fe:
                import logging as _log
                _log.getLogger("tradingagents").warning(
                    "LLM failover setup failed (%s) — using primary LLM only", _fe
                )
