"""Content-type classifier (opinion/news/analysis/question/other).

Ported from scripts/social_pipeline/v2/content_type.py.
"""
...
_HEADLINE_HINT = re.compile(r":\s+\S")
...
def classify(text: str) -> ContentType:
    ...
    is_first_person = bool(_FIRST_PERSON_EN.search(t) or _FIRST_PERSON_AR.search(t))
    if is_news_verb and not is_first_person:
        reasons.append("news-verb-3p")
        return ContentType("NEWS", WEIGHTS["NEWS"], reasons)
    if n_words <= 25 and _HEADLINE_HINT.search(t) and not is_first_person and is_news_verb:
        reasons.append("headline-pattern")
        return ContentType("NEWS", WEIGHTS["NEWS"], reasons)

    tech_hits = len(_TECH_TERMS.findall(t)) + len(_TECH_TERMS_AR.findall(t))
