...
# - core_stock_apis: open, high, low, close, volume, date (all required)
#   Volume must be explicitly flagged if missing. Liquidity flag required.
# =============================================================================
TOOLS_CATEGORIES = {
    "core_stock_apis": {
        "description": "OHLCV stock price data with mandatory fields",
        "mandatory_outputs": ["open", "high", "low", "close", "volume", "date"],
        "optional_outputs": ["low_liquidity", "volume_missing", "avg_daily_volume"],
        "notes": "EGX symbols use format SYMBOL.CA (e.g., COMI.CA). Daily bars only.",
        "tools": [
            "get_stock_data"
        ]
    },
    ...
    "social_media_data": {
        "description": "Social media sentiment and posts for EGX stocks (Arabic + English)",
        "tools": [
...
def route_to_vendor(method: str, *args, **kwargs):
    ...
    for vendor in fallback_vendors:
        ...
        if vendor_results:
            ...
            logger.warning(f"FAILED: Vendor '{vendor}' produced no results")

    # Final result summary
    if not results:
        logger.error(f"FAILURE: All {vendor_attempt_count} vendor attempts failed for method '{method}'")
        raise RuntimeError(f"All vendor implementations failed for method '{method}'")
    else:
        logger.info(f"FINAL: Method '{method}' completed with {len(results)} result(s) from {vendor_attempt_count} vendor attempt(s)")
