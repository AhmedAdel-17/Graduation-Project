...
# Read API key lazily at call time (NOT at import time) so that .env files
# loaded after this module is imported still take effect. Keeping the legacy
# module-level constant for backward compatibility, but every code path now
# uses _get_api_key() instead.
EODHD_API_KEY = os.getenv("EODHD_API_KEY", "")
EODHD_BASE_URL = "https://eodhd.com/api"


def _get_api_key() -> str:
    """Read EODHD_API_KEY from env at call time (handles late .env loads)."""
    return os.getenv("EODHD_API_KEY", "") or EODHD_API_KEY
