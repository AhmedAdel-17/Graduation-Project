...
def _extract_headlines_from_report(report: str) -> List[Dict[str, str]]:
    """
    Extract REAL news headlines from the LLM report — never the LLM's own
    reasoning text, JSON field names, or status / penalty messages.

    Strategy (most-trusted source first):
      1. Parse the structured JSON block and read ONLY `key_headlines[].headline`.
         These are the headlines the LLM explicitly tagged as real article titles.
      2. If the JSON block is missing/malformed, fall back to quoted strings and
         bullets — but heavily filter to drop anything that looks like a JSON key,
         a system message, or a confidence-adjustment string.

    Bug history: an earlier version regex-scraped every quoted string and bullet
    in the report. That fed text like "Silence in news coverage reduces
    confidence and suggests low market attention" back into FinBERT, which
    correctly scored words like "Silence", "errors", "incomplete" as bearish —
    creating a self-referential feedback loop that flipped sentiment to bearish
    when news was simply absent. Fix: only trust `key_headlines[].headline`.
    """
    headlines: List[Dict[str, str]] = []
    seen_texts: set = set()

    # ── 1. PRIMARY: structured JSON key_headlines (only trusted source) ──
    try:
        json_str = None
        ...
        if json_str:
            ...
            for item in data.get("key_headlines", []) or []:
                h_text = (item.get("headline") or "").strip()
                if h_text and len(h_text) > 15 and h_text not in seen_texts:
                    seen_texts.add(h_text)
                    headlines.append({
                        "text": h_text,
    ...
    # If the JSON path produced real headlines, we're done — DO NOT fall through
    # to the lossy quoted/bullet patterns (they introduce the feedback-loop bug).
    if headlines:
        return headlines

    # ── 2. FALLBACK: heuristic extraction with strict junk filtering ──
    # Used only when the LLM didn't emit a parseable JSON block at all.
    # Drops anything that looks like a JSON field name, status message, or
    # confidence-adjustment text.

    JUNK_PREFIXES = (
        "no news", "silence", "data source", "market news data also",
        "sparse news", "single-source", "single source", "confidence reduced",
        "limited coverage", "no specific risks", "no negative", "no positive",
        "insufficient data", "data unavailable", "headline:", "source:",
    )
    JUNK_KEYWORDS = (
        "confidence_score", "confidence_adjustments", "sentiment_strength",
        "key_headlines", "news_coverage", "risks_from_news",
        "catalysts_from_news", "explanation", "_score", "_adjustment",
    )
    ...
    def _looks_like_junk(text: str) -> bool:
        ...
        if not t:
            return True
        # Pure JSON field names: snake_case word with no spaces
        if " " not in t and ("_" in t or t.endswith("score")):
            return True
        if any(t.startswith(p) for p in JUNK_PREFIXES):
            return True
        if any(k in t for k in JUNK_KEYWORDS):
            return True
        # Likely an LLM penalty / reasoning line
        if "confidence" in t and ("→" in t or "->" in t):
            return True
        return False
    ...
    for h in quoted:
        h = h.strip()
        if 20 < len(h) < 300 and not _looks_like_junk(h) and h not in seen_texts:
            seen_texts.add(h)
    ...
    for b in bullets:
        b = b.strip()
        if 25 < len(b) < 300 and not _looks_like_junk(b) and b not in seen_texts:
            seen_texts.add(b)
            headlines.append({"text": b, "source": "bullet_in_report"})
...
First use the tools to retrieve news, then provide your analysis with the JSON summary."""
        ...
        if len(result.tool_calls) == 0:
            ...
            # GUARD A: If the analyst itself reports zero articles, do NOT run
            # the transformer at all. Anything regex-scrapes from the report
            # at this point is the LLM's own commentary about the absence of
            # news ("Silence reduces confidence...", "Absence of market news
            # prevents assessment..."), which FinBERT correctly reads as
            # bearish — producing a hallucinated bearish signal when the
            # honest answer is "no signal".
            #
            # GUARD B: Only run the transformer when extracted headlines came
            # from the structured JSON block. Heuristic-fallback headlines are
            # too noisy to feed into a sentiment model.
            headlines = (
                _extract_headlines_from_report(report)
                if total_articles > 0
                else []
            )
            if not headlines:
