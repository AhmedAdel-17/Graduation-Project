...
def check_leverage_violation(execution_plan: dict) -> Optional[RiskViolation]:
    """
    Detect ACTUAL leverage usage in the execution plan.

    Old behaviour (BUG): substring-matched "margin", "leverage", "2x", "3x"
    anywhere in the plan JSON. This produced false positives on benign banking
    and technical-analysis terminology:
      - "net interest margin"  (NIM — core banking metric)
      - "profit margin"        (income statement line)
      - "margin of safety"     (Graham/Buffett term)
      - "2x ATR stop"          (Wilder's standard ATR multiplier)
      - "2x risk-reward ratio" (institutional R:R standard)
      - "leverage ratio"       (regulatory measurement, not usage)

    New behaviour: two-stage check.
      1. Structural — look at known JSON fields that explicitly express
         leverage usage (margin_used, leverage_ratio, position_type, etc.)
      2. Textual — only flag bare "leverage" / "borrowed" / explicit "Nx
         leverage" patterns with word boundaries; skip when the surrounding
         context is clearly accounting / risk-measurement vocabulary.
    """
    if not EGX_RISK_LIMITS["no_leverage"]:
        return None

    # ── Stage 1: STRUCTURAL CHECK on known leverage fields ──────────────
    # These are explicit fields that, when set to leveraged values, indicate
    # actual leverage usage (not just discussion of leverage).
    def _walk(obj, path=""):
        """Recursively walk dict/list and yield (key_path, value) pairs."""
    ...
    LEVERAGE_FIELD_KEYS = (
        "margin_used", "margin_ratio", "leverage_ratio", "leverage_used",
        "borrowed_capital", "loan_amount", "position_leverage", "notional_leverage",
    )
    LEVERAGED_POSITION_TYPES = ("margin", "leveraged", "short", "borrowed", "futures")

    for key_path, val in _walk(execution_plan):
        key_lower = key_path.lower()

        # Field name explicitly indicates leverage
        if any(field in key_lower.split(".")[-1] for field in LEVERAGE_FIELD_KEYS):
            try:
                ...
                if num_val > 1.0:
                    return RiskViolation(
                        ...
                        explanation=(
                            f"Leverage is forbidden on EGX. Field '{key_path}' = {num_val} "
                            f"(must be ≤ 1.0)."
                        ),
                        remediation="Use 100% cash positions only.",
                    )
            except (TypeError, ValueError):
                # Non-numeric — fall through to text check
                pass

        # Position type field set to a leveraged value
        if key_lower.endswith(("position_type", "order_type", "instrument_type")):
            sval = str(val).lower()
            for forbidden in LEVERAGED_POSITION_TYPES:
                if forbidden == sval:  # exact match only (not substring)
                    return RiskViolation(
                        rule_name="LEVERAGE_FORBIDDEN",
                        severity="critical",
                        limit_value=1.0,
                        actual_value=2.0,
                        explanation=(
                            f"Leverage is forbidden on EGX. Field '{key_path}' = '{sval}' "
                            f"(forbidden type)."
                        ),
                        remediation="Use spot/cash limit orders only.",
                    )

    # ── Stage 2: TEXTUAL CHECK with strict context ──────────────────────
    plan_text = json.dumps(execution_plan).lower()

    # Word-boundary regex patterns that target ACTUAL leverage usage.
    # We exclude the common banking / risk-measurement vocabulary that
    # legitimately uses these words.
    BENIGN_CONTEXT = (
        "profit margin", "net interest margin", "operating margin",
        "gross margin", "ebitda margin", "net margin",
        "margin of safety", "margin expansion", "margin compression",
        "leverage ratio",          # regulatory measurement
        "operational leverage",    # business metric
        "financial leverage ratio",
        "debt-to-equity",
        "2x atr", "3x atr", "2×atr", "3×atr",   # standard stop-loss multipliers
        "2x risk", "3x risk",      # R:R ratios
        "risk-reward",
    )

    # If the plan only mentions these benign contexts, skip text-based veto.
    # We strip benign phrases out and then re-scan for bare leverage talk.
    sanitized = plan_text
    for benign in BENIGN_CONTEXT:
        sanitized = sanitized.replace(benign, "")

    # Now look for explicit leverage patterns with word boundaries
    explicit_patterns = [
        (r"\bborrowed\s+capital\b",     "borrowed capital"),
        (r"\busing\s+margin\b",          "using margin"),
        (r"\bbuy\s+on\s+margin\b",       "buy on margin"),
        (r"\bmargin\s+account\b",        "margin account"),
        (r"\bmargin\s+loan\b",           "margin loan"),
        (r"\b\d+x\s+leverage\b",         "Nx leverage"),
        (r"\bleveraged\s+(position|etf|product)\b", "leveraged instrument"),
        (r"\bshort\s+(sell|position)\b", "short selling"),
    ]
    ...
    for pattern, label in explicit_patterns:
        if re.search(pattern, sanitized):
            return RiskViolation(
                ...
                limit_value=1.0,
                actual_value=2.0,
                explanation=f"Leverage is forbidden on EGX. Found pattern: '{label}'.",
