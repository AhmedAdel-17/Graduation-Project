...
def test_structured_output_extraction():
    """Test JSON/text extraction from LLM-like responses via _extract_narrative."""
    from tradingagents.agents.analysts.social_media_analyst import _extract_narrative
    ...
    ```json
    {"narrative": "COMI shows strong retail buzz driven by earnings beat.", "cited_post_ids": ["p1", "p2"]}
    ```

    The stock shows positive momentum.
    '''

    result = _extract_narrative(text_with_json)
    assert "COMI" in result["narrative"]
    assert result["cited_post_ids"] == ["p1", "p2"]

    # Test with plain text (no JSON block) — first paragraph used as narrative
    text_plain = "Overall sentiment is bullish with strong retail interest.\n\nSecond paragraph here."
    result = _extract_narrative(text_plain)
    assert "bullish" in result["narrative"].lower()
    assert result["cited_post_ids"] == []

    # Test with no meaningful content
    text_empty = "   "
    result = _extract_narrative(text_empty)
    assert isinstance(result["narrative"], str)
    assert isinstance(result["cited_post_ids"], list)
